...
const Cart = () => {
    ...
    const removeItem = (item) => () => {
        if (!window.confirm("Delete Item?")) return //eta ekta window object er item jeta hosse amake aghe confirm korte hoy.yes click korle porer portion e continue kore nahole function return kore
        ...
        <Layout title="Your Cart" description="Hurry up! Place your order!" className="container">
            ...
            <div className="container my-5">
                <table className="table table-hover">
                    ...
                    <tbody>
                        {cartItems.map((item, i) => <CartItem //i(index) niyechi serial no generate korar jonno
                            item={item}
                            serial={i + 1}
                            key={item._id}
